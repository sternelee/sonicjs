...
import { autoRegisterCollectionDocumentTypes } from '../../services/document-types-seed'
...
describe('api-content-crud → documents (decommission step)', () => {
  ...
  it('POST creates a document (not a content row), published, and GET /:id resolves it', async () => {
    const res = await app.request('/api/content', json('POST', { collectionId: 'news-id', title: 'Hello', slug: 'hello', status: 'published', data: { body: 'x' } }))
    expect(res.status).toBe(201)
    const created = (await res.json()).data
    expect(created.collectionId).toBe('news-id')

    // stored as a published news document
    const doc = db.raw.prepare("SELECT * FROM documents WHERE type_id='news' AND slug='hello'").get()
  ...
  it('rejects a duplicate slug with 409', async () => {
    await app.request('/api/content', json('POST', { collectionId: 'news-id', title: 'A', slug: 'dup', data: {} }))
    const res = await app.request('/api/content', json('POST', { collectionId: 'news-id', title: 'B', slug: 'dup', data: {} }))
    expect(res.status).toBe(409)
  })

  it('PUT saves a new draft and republishes it', async () => {
    const created = (await (await app.request('/api/content', json('POST', { collectionId: 'news-id', title: 'V1', slug: 'v', status: 'published', data: {} }))).json()).data
  ...
  it('DELETE soft-deletes every version row of the root', async () => {
    const created = (await (await app.request('/api/content', json('POST', { collectionId: 'news-id', title: 'Del', slug: 'del', data: {} }))).json()).data
