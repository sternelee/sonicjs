...
> **Note:** The `create-sonicjs` scaffolding already includes a Blog Posts collection at `src/collections/blog-posts.collection.ts`. This guide uses that existing collection — no need to create one manually. The collection is registered in `src/index.ts` via `registerCollections()` and stored in the in-memory CollectionRegistry; `autoRegisterCollectionDocumentTypes` registers a `document_type` row per collection at bootstrap.
