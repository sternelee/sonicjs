...
```typescript
import { definePlugin, HOOKS } from '@sonicjs-cms/core'
...
const myPlugin = definePlugin({
  ...
  onBoot(ctx) {
    // Register hooks here
    ctx.hooks.register(HOOKS.CONTENT_SAVE, async (data) => {
      data.lastModified = Date.now()
      return data
    }, 5)

    ctx.hooks.register(HOOKS.AUTH_LOGIN, loginHandler)
  }
})
```
...
const myPlugin = definePlugin({
  ...
  onBoot(ctx) {
    ...
    ctx.hooks.register('my-plugin:item-processed', async (data) => {
