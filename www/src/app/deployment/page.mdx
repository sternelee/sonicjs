...
This guide covers **Cloudflare deployment** — the recommended path for global distribution. If you need to run SonicJS on your own server or in Docker without a Cloudflare account, see the [Self-Hosting guide](/self-hosting).
...
# Admin user is created automatically at bootstrap.
# Set these secrets before first deploy:
echo "admin@yourdomain.com" | wrangler secret put ADMIN_EMAIL --env production
echo "your-secure-password" | wrangler secret put ADMIN_PASSWORD --env production

# If using the seed script, demo credentials are:
...
- [Self-Hosting Guide](/self-hosting) - Run SonicJS on your own server with Docker or Node.js
