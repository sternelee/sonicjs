...
`listFields` controls which fields appear as columns in the admin content list. Without it, the list shows only the document title.
...
Order the array to match your preferred column order. Omit rich-text and large JSON fields — they're not scannable in a table cell.

`searchFields` works the same way for the admin search bar:
