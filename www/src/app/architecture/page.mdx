...
export function bootstrapMiddleware() {
  return async (c: Context<{ Bindings: Bindings }>, next: Next) => {
    ...
    try {
      ...
      // 2. Self-heal q_* generated columns
      await migrationService.ensureDocumentGeneratedColumns()

      // 3. Register collections and document types
...
JWT (auth_token cookie + Bearer header) + Better Auth session; `c.get('user') = { userId, email, role }`:
...
- **Core Plugins** (registered via `definePlugin()` + `registerPlugins()`): lexical-editor (default), media, cache, database-tools
- **Optional Plugins**: FAQ, demo-login
