...
SonicJS telemetry collects the following anonymous data during install-time only (sent once during initial setup):
...
Events are sent to: `https://stats.sonicjs.com/v1/events`
...
Set `DO_NOT_TRACK=1` in your environment to disable telemetry. You can also set the `SONICJS_TELEMETRY` environment variable to `false`:
