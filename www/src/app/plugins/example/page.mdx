...
Plugin files are copied into your project when you run `npm create sonicjs@latest`. Unlike `@sonicjs-cms/core`, they are **your code** — `npm install` won't update them. To pull the latest version of the example plugin from the SonicJS main branch:

```bash
npm run update:plugin
# or specify the plugin name explicitly:
npm run update:plugin -- example
```
...
The script downloads each file listed in its manifest from `github.com/SonicJs-Org/sonicjs` and overwrites your local copies.

<Callout type="warning">
**Commit or stash local changes first.** `update:plugin` overwrites files — any custom modifications to the example plugin will be lost.
</Callout>
...
# Commit if happy
git add src/plugins/example && git commit -m "chore(plugins): update example plugin"
```

To add new plugins to the manifest (for when additional bundled plugins are released), edit `scripts/update-plugin.ts` and add an entry to `PLUGIN_MANIFEST`.
