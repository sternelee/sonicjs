export const sections = [
  ...
  { title: 'Example Plugin Reference', id: 'example-plugin-reference' },
...
- `collections/moods.collection.ts` — `CollectionConfig` for mood entries
...
3. **Settings schema** — `configSchema` with Zod defines the Settings tab form
4. **Collection registration** — `moodsCollection` imported and passed to `registerCollections()` in `src/index.ts`
...
<div className="not-prose mt-2">
  <Button href="/content/blog/tutorials/building-your-first-sonicjs-plugin" variant="text" arrow="right">
    <>Blog: Building Your First SonicJS Plugin (step-by-step walkthrough)</>
  </Button>
</div>
