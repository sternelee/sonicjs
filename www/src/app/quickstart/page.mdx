...
1. Navigate to **Content**, then pick a collection from the **New Content** dropdown
...
# Create content
curl -X POST http://localhost:8787/api/blog_posts \
  -H "Content-Type: application/json" \
...
// Create content
const response = await fetch('http://localhost:8787/api/blog_posts', {
  method: 'POST',
  headers: {
...
curl http://localhost:8787/api/blog_posts/:id
```
