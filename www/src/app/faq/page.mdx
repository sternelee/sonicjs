...
- Modern tech stack (Hono.js, Document Model on D1, HTMX admin UI)
...
```bash
# Create a new SonicJS app (2 minutes)
...
- **Collection Registry** - code-defined via registerCollections() (read-only list at /admin/collections)
- **Dashboard** - Analytics and system overview
- **User Management** - Role-based access control

The admin interface is built with HTMX and server-rendered templates and accessible at `/admin`.
...
**Component Overrides:**
Override admin templates / add admin pages via plugins.
...
Creating a plugin is straightforward with `definePlugin` and `registerPlugins`:

<CodeGroup title="Basic Plugin">

```typescript
import { definePlugin, registerPlugins } from '@sonicjs-cms/core'

const myPlugin = definePlugin({
  name: 'my-plugin',
  register(app) {
    app.get('/custom', (c) => c.json({ message: 'Hello!' }))
  }
})
...
// Require authentication for routes using middleware
app.get('/api/protected', requireAuth, async (c) => {
  // handler here
})

// Access authenticated user — c.get('user') = { userId, email, role }
...
// Check permissions
app.post('/admin-action', requireAuth, async (c) => {
  const user = c.get('user')
