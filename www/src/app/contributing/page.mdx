...
| Command | Description |
|---------|-------------|
| `npm run db:generate` | Generate migration files from schema changes (legacy auth tables only; document schema is in raw migrations) |
| `npm run db:migrate` | Apply migrations to local D1 database |
| `npm run db:migrate` | Apply migrations to production database |
| `npm run db:studio` | Open Drizzle Studio (database GUI) (legacy auth tables only) |
...
| Command | Description |
|---------|-------------|
| `npm run plugins:generate` | Regenerate plugin registry from manifests (v2 only — v3 plugins use definePlugin() + registerPlugins(), no manifest files) |
| `npm run plugins:watch` | Watch plugin manifests and auto-regenerate (v2 only — v3 plugins use definePlugin() + registerPlugins(), no manifest files) |
...
- 💬 [Discord](https://discord.gg/SJArGfJgPJ)
